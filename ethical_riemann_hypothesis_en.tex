...
\begin{itemize}
    ...
    \item \textbf{Real-world data}: Adult Income (UCI) and COMPAS (ProPublica) datasets can be obtained via \texttt{scripts/fetch\_real\_data.sh}, which downloads \texttt{adult.data}, \texttt{adult.test} into \texttt{data/real\_world/} and \texttt{compas-scores-two-years.csv} into \texttt{data/}. Run \texttt{scripts/convert\_adult\_to\_csv.py} to convert UCI Adult format into \texttt{data/adult.csv} for the Adult Income case study. The COMPAS dataset is used directly by \texttt{simulation/real\_data/compas\_case\_study.py}.
...
System coherence & 0.9570 \\
Von Neumann entropy $H(\rho)$ & 0.2528 \\
\bottomrule
\end{tabular}
\end{table}
