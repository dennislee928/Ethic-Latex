...
\begin{itemize}
    ...
    \item \textbf{Experimental repeatability}: Although the results presented here are from a single representative run, we verified statistical stability across independent runs (different random seeds). The coefficient of variation (standard deviation/mean) of error growth exponent $\alpha$ is less than $0.15$, indicating good consistency. Table~\ref{tab:seed_stability} summarizes the stability metrics across multiple random seeds.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Data outputs}: Numerical results from simulations (including statistical metrics for each judge, error term sequences, spectrum data, stability-of-$\alpha$ tables across random seeds, etc.) are saved in the \texttt{simulation/output/} directory, including:
    \begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Figure generation}: All paper figures are automatically generated by the \texttt{simulation/generate\_all\_figures.py} script. Executing this script will completely reproduce all figures. Figures are saved in PDF format in the \texttt{simulation/output/figures/} directory and then copied into the \texttt{figures/} directory used by \LaTeX.
...
\subsection{Summary of Error Behavior Across Judge Types}

Before turning to detailed per-judge narratives, Table~\ref{tab:summary_judges}
summarizes the key quantitative indicators for each judgment system.
...
\begin{figure}[htbp]
  ...
  \caption{Error growth analysis showing $|E(x)|$ vs. complexity $x$ in log-log scale. Linear regression yields an error growth exponent $\alpha = -0.629$ ($R^2 = 0.604$). The negative exponent indicates that error decreases as complexity increases, which is mathematically superior to the ERH-style worst-case bound of $\alpha \approx 0.5$. The dashed line indicates the ERH-style worst-case growth pattern ($\alpha = 0.5$) for comparison.}
...
To illustrate how ERH can be used as an auditing tool beyond synthetic simulations, we conducted a small real-data case study on the UCI Adult Income dataset.\footnote{For reproducibility and anonymized review, we use a preprocessed CSV version of the Adult dataset stored locally; the code for this experiment is provided in \texttt{simulation/real\_data/adult\_income\_case\_study.py}.}
...
\begin{table}[h]
\centering
\caption{Comparison of baseline and mitigated models on Adult Income dataset. Accuracy is overall classification accuracy. Group gap (ΔFPR) measures the difference in false positive rates between protected groups. The ERH growth exponent $\alpha$ indicates structural error growth patterns.}
\label{tab:real_data_comparison}
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \textbf{Accuracy} & \textbf{Group Gap (ΔFPR)} & \textbf{$\alpha$} & \textbf{Comment} \\
\midrule
...
\subsection{Psychohistory as a Macro-Ethical Perspective on ERH}
\label{subsec:psychohistory}

\subsubsection{Motivation and Fiction--Science Boundary}

Isaac Asimov's notion of \emph{psychohistory} in the \emph{Foundation} series imagines a mathematical science that predicts the macro-scale behaviour of societies.
Our use of psychohistory is strictly metaphorical: we do not claim to build a real Seldon Plan, nor to predict history in a literal sense.
Rather, psychohistory offers a narrative device for thinking about macro-level regularities, crises, and interventions in complex moral systems.
In this paper, ERH provides a rigorous mathematical metaphor for error growth in complexity space, while psychohistory provides a complementary fictional metaphor for thinking about macro-scale predictability and control.

\subsubsection{Core Ideas of Psychohistory}

Four aspects of psychohistory are particularly relevant to ERH:
\begin{itemize}
    \item \textbf{Law of large numbers for societies}: individual behaviour is unpredictable, but large populations exhibit statistical regularities.
    \item \textbf{Macro-level social dynamics}: aggregate flows can be described by coarse-grained variables, akin to ``social physics.''
    \item \textbf{Crises and Seldon Plans}: history contains critical junctures where small interventions have outsized impact.
    \item \textbf{The Mule and limits of prediction}: rare, highly atypical agents or events can disrupt long-range forecasts.
\end{itemize}
These ideas map naturally onto questions about error distributions, phase transitions, and tail events in AI-mediated moral orders.

\subsubsection{Conceptual Mapping: Psychohistory $\leftrightarrow$ ERH}

The conceptual bridge between psychohistory and ERH can be summarized as follows:
\begin{itemize}
    \item \textbf{Individual vs.\ aggregate}: psychohistory treats individuals as noise but societies as predictable; ERH treats individual misjudgments as messy but ethical primes as exhibiting structured aggregate patterns.
    \item \textbf{Statistical regularity vs.\ error bounds}: psychohistory relies on statistical regularities over time; ERH relies on sublinear bounds on $E(x)$ over complexity.
    \item \textbf{Seldon Plans vs.\ ERH bounds}: Seldon Plans specify allowable deviations of historical trajectories; ERH specifies allowable growth of structural error before we declare a judgment system ``unhealthy.''
    \item \textbf{The Mule vs.\ ERH violations}: psychohistory's Mule corresponds to catastrophic ERH violations---sudden bursts where $|E(x)|$ becomes much larger than expected, signalling unmodelled structure or genuine regime change.
\end{itemize}
In short, psychohistory offers a time--society metaphor, whereas ERH offers a complexity--error metaphor for the same underlying concern: when can macro-level patterns be trusted?

\subsubsection{A Macro-Dynamic Extension Sketch}

We can sketch, without fully developing, a macro-dynamic extension in which $E(x)$ becomes $E(x,t)$, tracking how structural error evolves over both complexity $x$ and time $t$.
In such a setting, ethical ``crisis points'' would correspond to times $t^\ast$ at which $E(x,t)$ exhibits sudden acceleration across a range of high-complexity $x$, analogous to Seldon crises.
Potential tools for modelling $E(x,t)$ include agent-based simulations, network dynamics, and fluid or PDE-style models on the $(x,t)$ plane.
We do not pursue this full psychohistorical model here, but view ERH as one coordinate projection of a richer macro-dynamic theory of ethical error.

\subsubsection{Implications for AI Ethics and Governance}

Interpreted through the psychohistory metaphor, ERH suggests a style of \emph{macro-monitoring} for AI systems:
instead of only auditing individual decisions, regulators and oversight bodies can track how structural error indicators (e.g., ethical prime counts, growth exponents) behave over time and complexity.
Spikes in $E(x,t)$ for high-stakes, high-complexity regions can serve as early-warning signals---the analogue of Seldon crises---that trigger audits, human review, or policy interventions.
This perspective also motivates ``Second Foundation''-like oversight structures that adjust models and governance rules at the macro level, aiming to keep systems within acceptable ERH bands rather than micromanaging every decision.
In our implementation, psychohistory is therefore not invoked as a literal claim about predicting human history, but as a metaphorical and modelling device for reasoning about macro-level error structures and crisis dynamics in AI-mediated moral systems.
Our integration tests (covering variations in agent counts, network topologies, and temporal horizons, as summarized in the psychohistory test reports) indicate that coupling the ERH-based analysis with this psychohistory-inspired macro layer does not break the sublinear error-growth behavior: the aggregate error growth exponents remain well below the ERH-style worst-case target.

\textbf{Operational Recommendations for AI Governance:}
\begin{itemize}
    \item \textbf{Regular ERH-style monitoring}: Regulatory bodies and oversight organizations should periodically monitor ERH-style indicators (error growth exponents, ethical prime counts) as part of routine AI system audits, tracking how these metrics evolve over time and across complexity ranges.
    \item \textbf{Early-warning dashboards}: Design ``early-warning dashboard'' systems that monitor $E(x,t)$ in high-complexity regions, with automated alerts when error growth patterns approach or exceed ERH-style worst-case bounds, serving as the analogue of Seldon crises that trigger human review or policy interventions.
    \item \textbf{Macro-structural KPIs}: Incorporate ERH-style bounds as ``macro-structural KPIs'' in regulatory frameworks, complementing traditional pointwise accuracy and fairness metrics. This provides a quantitative criterion for structural health that can be evaluated alongside domain-specific quality measures.
    \item \textbf{Second Foundation oversight structures}: Establish macro-level oversight mechanisms (analogous to Asimov's Second Foundation) that adjust models and governance rules at the system level, maintaining acceptable ERH bands rather than micromanaging individual decisions.
\end{itemize}

\subsubsection{Limits of the Metaphor and Future Hybrid Work}

The psychohistory metaphor has clear limits.
Psychohistory is a work of science fiction, not a validated theory of society, and ERH itself is an idealized formal framework rather than a predictive law of history.
Combining the two risks over-claiming if we forget that both are, at best, disciplined ways of thinking about structure, crisis, and intervention.
Nevertheless, the metaphor is useful precisely because it forces us to articulate what it would mean for an AI-mediated moral order to exhibit macro-level regularities, crisis points, and structural vulnerabilities.
Future work on hybrid ERH--psychohistory models (e.g., agent-based simulations with ERH-style observables) can make this connection more concrete, but in this paper we confine ourselves to the conceptual bridge and its implications for AI governance.

\subsection{Limitations and Threats to Validity}

We acknowledge several limitations and threats to validity:
\begin{itemize}
    \item \textbf{Model idealization}: Our simulations use synthetic actions, stylized judgment systems, and explicitly specified ``true'' moral values $V(a)$. In real-world settings, $V(a)$ is inaccessible, and both complexity and importance are noisy, contested constructs.
    \item \textbf{Heuristic analogy to prime numbers}: The analogy with prime numbers is heuristic rather than exact. Ethical primes borrow the language of prime distributions without claiming that moral errors literally obey theorems from analytic number theory.
    \item \textbf{Subjective design choices}: The framework assumes that complexity can be quantified and that ethical primes can be selected via importance quantiles and structural criteria. These design choices are philosophically and practically debatable, and alternative operationalizations may yield different prime sets.
    \item \textbf{ERH as a necessary but not sufficient condition}: The ERH-style structural bound is, at best, a necessary condition for a morally acceptable system, not a sufficient one. As the Conservative Judge illustrates, a system can have error growth well below the ERH-style worst-case bound while still exhibiting an unacceptably high overall mistake rate. This aligns with the formal statement in Section~\ref{rem:erh_necessary}: satisfying the ERH-style bound ensures structural health in terms of error growth patterns, but must be complemented by low overall error rates and absence of systematic biases to constitute an acceptable moral judgment system.
    \item \textbf{Psychohistory integration at conceptual level}: The psychohistory layer is used only at the level of conceptual modelling and simulation. It is not a validated theory of society or history, and we do not claim real-world predictive power; its role is to structure thinking about macro-level error patterns and crises. The current psychohistory-inspired simulations are exploratory and reside mainly in the supplementary code, and we do not claim any empirical validation as a theory of social dynamics.
\end{itemize}
