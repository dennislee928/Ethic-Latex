...
def find_figures(figures_dir):
    ...
    figure_map = {
        ...
        'compas_erh_bound.png': {'label': 'fig:compas_erh_bound', 'caption_en': 'COMPAS adherence to Riemann bound. Log-log plot of $|E(x)|$ vs. complexity $x$ with theoretical bound $C\\cdot x^{0.5}$. Error growth exponent $\\alpha\\approx-0.20$ indicates sublinear growth consistent with ERH.', 'caption_zh': 'COMPAS 與 Riemann 界線的符合度。$|E(x)|$ 與複雜度 $x$ 的對數-對數圖，含理論界線 $C\\cdot x^{0.5}$。誤差增長指數 $\\alpha\\approx-0.20$ 表示與 ERH 一致的次線性增長。', 'section': 'compas'},
        'alpha_comparison_bar.png': {'label': 'fig:alpha_comparison_bar', 'caption_en': 'Structural similarity: $\\alpha$ comparison across Radical, Conservative (synthetic), COMPAS, and Adult Income (real-world). COMPAS $\\approx-0.20$ highlighted.', 'caption_zh': '結構相似性：Radical、Conservative（模擬）、COMPAS、Adult Income（真實）的 $\\alpha$ 比較。COMPAS $\\approx-0.20$ 標示。', 'section': 'compas'},
