...
def find_figures(figures_dir):
    ...
    figure_map = {
        ...
        'alpha_comparison_real_vs_simulated.png': {'label': 'fig:alpha_real_vs_simulated', 'caption_en': 'Real-world growth exponent $\\alpha$ versus synthetic reference judges. Compares the Adult Income and COMPAS case studies with a simulated Conservative judge to contextualize structural error-growth regimes.', 'caption_zh': '真實世界增長指數 $\\alpha$ 與模擬參考判斷者之比較。將 Adult Income 與 COMPAS 案例研究和模擬保守判斷者對照，以脈絡化不同的結構性誤差增長區間。', 'section': 'compas'},
        'compas_erh_bound.png': {'label': 'fig:compas_erh_bound', 'caption_en': 'COMPAS under an ERH-style bound. Log-log plot of $|E(x)|$ versus complexity $x$ with theoretical reference line $C\\cdot x^{0.5}$. The fitted exponent $\\alpha\\approx-0.20$ indicates bounded sublinear error growth under the chosen proxy, not fairness by itself.', 'caption_zh': 'ERH 式界線下的 COMPAS。$|E(x)|$ 與複雜度 $x$ 的對數-對數圖，含理論參考線 $C\\cdot x^{0.5}$。擬合指數 $\\alpha\\approx-0.20$ 表示在所選 proxy 下屬於有界的次線性誤差增長，而非單獨構成公平性結論。', 'section': 'compas'},
        'alpha_comparison_bar.png': {'label': 'fig:alpha_comparison_bar', 'caption_en': 'Comparison of fitted growth exponents across Radical, Conservative (synthetic), COMPAS, and Adult Income case studies. COMPAS $\\approx-0.20$ is highlighted to situate the real-data result relative to synthetic baselines.', 'caption_zh': 'Radical、Conservative（模擬）、COMPAS 與 Adult Income 案例研究之擬合增長指數比較。特別標示 COMPAS $\\approx-0.20$，以對照真實資料結果與模擬基線。', 'section': 'compas'},
        ...
        'normalized_error_oscillation.png': {'label': 'fig:normalized_oscillation', 'caption_en': 'Normalized error oscillation $E(x)/\\sqrt{x}$ with confidence interval for COMPAS. Useful for checking bounded behavior under the ERH-style normalization, not as a fairness metric.', 'caption_zh': 'COMPAS 的正規化誤差振盪 $E(x)/\\sqrt{x}$ 與信賴區間。可用於檢查 ERH 式正規化下是否保持有界，但不應視為公平性指標。', 'section': 'supplementary'},
