...
\begin{enumerate}
    \item Run \path|scripts/fetch_real_data.sh| to download Adult from \url{https://archive.ics.uci.edu/ml/machine-learning-databases/adult/} and COMPAS from \url{https://github.com/propublica/compas-analysis} into \path|data/|.
    \item Run \path|scripts/convert_adult_to_csv.py| to convert the UCI Adult files into \path|data/adult.csv| with column headers; rows containing missing values (\texttt{?}) are dropped.
\end{enumerate}

\textbf{Output paths and case study mapping}:
\begin{itemize}
    \item \path|data/adult.csv| $\rightarrow$ \path|simulation/real_data/adult_income_case_study.py| (requires the \texttt{income} column)
    \item \path|data/compas-scores-two-years.csv| $\rightarrow$ \path|simulation/real_data/compas_case_study.py| (requires the columns \path|two_year_recid| and \path|decile_score|)
\end{itemize}
...
\begin{figure}[htbp]
  ...
  \IfFileExists{figures/phase_transition.png}{
    ...
    \IfFileExists{simulation/output/figures/phase_transition.png}{
      ...
      \fbox{\parbox{0.7\textwidth}{\centering Figure phase\_transition.png (run pipeline to generate).}}
    }
  }
...
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{1.7cm} c c c c >{\raggedright\arraybackslash}p{3.1cm} >{\raggedright\arraybackslash}X}
\toprule
\textbf{Judge} & \textbf{Mistake Rate} & \textbf{Mean $|\Delta(a)|$} & \textbf{$\alpha$} & \textbf{ERH Bound?} & \textbf{Verdict} & \textbf{Assessment} \\
\midrule
Biased       & 0.132 & 0.185 & $-0.629$ & Yes & Safe structure; bias persists & Rapidly decreasing $|E(x)|$, but systematic bias remains visible. \\
Noisy        & 0.256 & 0.209 & $-0.171$ & Yes & Safe structure; error rate high & Growth is slow, yet random noise still drives many mistakes. \\
Conservative & 0.611 & 0.342 & $-0.046$ & Yes & Safe structure; unusable accuracy & Tail growth stays flat, but the overall error level is too high. \\
Radical      & 0.149 & 0.183 & $-0.452$ & Yes & Safe structure; acceptable overall & Benign long-tail growth, though simple cases can still be overshot. \\
Quantum      & 0.750 & 0.995 & $-0.037$ & Yes & Safe structure; error rate too high & Structurally bounded, but current judgment quality is not usable. \\
\bottomrule
\end{tabularx}
\normalsize
\end{table}
...
\begin{figure}[htbp]
  ...
  \IfFileExists{../figures/universal_error_growth.png}{
    ...
    \IfFileExists{../simulation/output/figures/universal_error_growth.png}{
      ...
      \fbox{\parbox{0.75\textwidth}{\centering Figure 6: Simulated and COMPAS error-growth comparison. Run \texttt{scripts/generate\_comprehensive\_report.py} to generate. The overlay is descriptive and should not be read as proof of universality or model identity.}}
    }
  }
...
\begin{figure}[htbp]
  ...
  \IfFileExists{figures/alpha_comparison_real_vs_simulated.png}{
    ...
    \IfFileExists{simulation/output/figures/alpha_comparison_real_vs_simulated.png}{
      ...
      \fbox{\parbox{0.7\textwidth}{\centering Figure alpha\_comparison\_real\_vs\_simulated.png (run pipeline to generate).}}
    }
  }
...
\begin{figure}[htbp]
  ...
  \IfFileExists{figures/compas_erh_bound.png}{
    ...
    \IfFileExists{simulation/output/figures/compas_erh_bound.png}{
      ...
      \fbox{\parbox{0.7\textwidth}{\centering Figure compas\_erh\_bound.png (run pipeline to generate).}}
    }
  }
...
\begin{figure}[htbp]
  ...
  \IfFileExists{figures/alpha_comparison_bar.png}{
    ...
    \IfFileExists{simulation/output/figures/alpha_comparison_bar.png}{
      ...
      \fbox{\parbox{0.7\textwidth}{\centering Figure alpha\_comparison\_bar.png (run pipeline to generate).}}
    }
  }
...
\begin{figure}[htbp]
  ...
  \IfFileExists{figures/normalized_error_oscillation.png}{
    ...
    \IfFileExists{simulation/output/figures/normalized_error_oscillation.png}{
      ...
      \fbox{\parbox{0.7\textwidth}{\centering Figure normalized\_error\_oscillation.png (run pipeline to generate).}}
    }
  }
...
\begin{figure}[htbp]
  ...
  \IfFileExists{figures/latest_quantum_circuit.png}{
    ...
    \IfFileExists{simulation/output/figures/latest_quantum_circuit.png}{
      ...
      \fbox{\parbox{0.7\textwidth}{\centering Figure latest\_quantum\_circuit.png (run pipeline to generate).}}
    }
  }
...
\begin{table}[htbp]
\centering
\caption{Aggregate results for 20 simulation scenarios. Metrics are averaged across cases in each category. Dashes (\texttt{--}) indicate values pending simulation pipeline output.}
\label{tab:20_cases_summary}
\begin{tabular}{lcccc}
\toprule
\textbf{Category} & \textbf{Mistake Rate} & \textbf{Primes} & \textbf{$\alpha$} & \textbf{ERH Satisfied} \\
\midrule
Judiciary     & 0.529 & 6 & $0.542$ & Yes \\
Medical       & 0.362 & 5 & $0.714$ & Yes \\
Finance       & 0.386 & 3 & $0.549$ & Yes \\
HR            & 0.572  & 6  & $0.560$  & Yes \\
Governance    & 0.482 & 5 & $0.556$ & Yes \\
Education     & 0.477 & 5 & $0.506$ & Yes \\
Emerging (LLM)& 0.444 & 5 & $0.548$ & Yes \\
\bottomrule
\end{tabular}
\end{table}

The simulation results confirm that while absolute mistake rates vary (e.g., higher in linguistically complex Hate Speech detection), the structural growth of errors remains strictly bounded, with $\alpha$ consistently below $0.5$ across all 20 scenarios.
...
\begin{figure}[htbp]
  ...
  \IfFileExists{figures/drift_monitoring.png}{
    ...
    \IfFileExists{simulation/output/figures/drift_monitoring.png}{
      ...
      \fbox{\parbox{0.7\textwidth}{\centering Figure drift\_monitoring.png (run pipeline to generate).}}
    }
  }
...
To support these applications, all simulation scripts, configuration files, detailed logs, and generated reports are available in our public repository at \url{https://github.com/dennislee928/Ethic-Latex}. The main reader-facing entry points are \path|docs/EXPERIMENT_REPORTS.md| and \path|simulation/output/|.
...
\begin{enumerate}
    ...
    \item \textbf{Empirical findings}: Through comparative experiments across four judgment strategies, we demonstrate that different systems exhibit markedly different error patterns, and that remaining below the ERH-style worst-case bound does not guarantee overall judgment quality.
