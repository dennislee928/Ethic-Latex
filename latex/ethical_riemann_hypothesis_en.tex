...
\textbf{What ERH lets us conclude:} the mitigated model is less prone to runaway accumulation of errors on harder examples under this complexity proxy. \textbf{What ERH does not let us conclude:} that the intervention is fair in every subgroup or acceptable under every harm model. \textbf{Practical takeaway:} ERH is useful here as a complement to standard fairness metrics because it reveals whether an intervention improves the structure of long-tail errors, not just average performance.
...
\textbf{What ERH lets us conclude:} COMPAS appears structurally stable with respect to this specific growth metric, because the fitted exponent remains well below the ERH-style worst-case threshold of $0.5$. \textbf{What ERH does not let us conclude:} demographic fairness, calibration quality, legal legitimacy, or the causal source of the observed disparities. \textbf{Practical takeaway:} ERH is useful here as a screening layer for complexity-linked error accumulation, after which subgroup disparity and calibration analyses still need to do the substantive fairness work.
...
Based on the findings of this study, several bounded directions merit further exploration:

\begin{itemize}
    \item \textbf{Measurement robustness}: test alternative complexity proxies, threshold choices, and subgroup-conditioned estimates to determine when fitted exponents are stable enough to support audit use.
    \item \textbf{Empirical audit studies}: replicate Adult- and COMPAS-style analyses on additional documented decision systems, pairing ERH with calibration, fairness, and domain-specific harm metrics rather than treating it as a standalone score.
    \item \textbf{Theory and estimation}: tighten finite-sample error analysis, confidence intervals, and explicit assumptions under which ERH-style bounds should or should not be expected.
\end{itemize}
