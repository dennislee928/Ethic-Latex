...
In addition to Adult Income, we retain two further methodological case studies: one on \emph{university entrance exam cheating} and one on \emph{sexual abuse reporting}.\footnote{The corresponding scripts are located in \texttt{simulation/real\_data/exam\_cheating\_case\_study.py} and \texttt{simulation/real\_data/sexual\_abuse\_case\_study.py}. Both are designed for anonymized or synthetic data only.}
At present these should be read as portability demonstrations for the analysis workflow rather than as additional empirical validation. Their value is in showing how to ask whether critical errors keep accumulating with complexity in high-risk settings; deployment conclusions would still require domain-specific data quality checks, harm models, and ordinary fairness review.
...
\midrule
Judiciary     & [CASE_JUD_MR] & [CASE_JUD_PR] & [CASE_JUD_AL] & [CASE_JUD_ERH] \\
Medical       & [CASE_MED_MR] & [CASE_MED_PR] & [CASE_MED_AL] & [CASE_MED_ERH] \\
Finance       & [CASE_FIN_MR] & [CASE_FIN_PR] & [CASE_FIN_AL] & [CASE_FIN_ERH] \\
HR            & [CASE_HR_MR]  & [CASE_HR_PR]  & [CASE_HR_AL]  & [CASE_HR_ERH] \\
Governance    & [CASE_GOV_MR] & [CASE_GOV_PR] & [CASE_GOV_AL] & [CASE_GOV_ERH] \\
Education     & [CASE_EDU_MR] & [CASE_EDU_PR] & [CASE_EDU_AL] & [CASE_EDU_ERH] \\
Emerging (LLM)& [CASE_LLM_MR] & [CASE_LLM_PR] & [CASE_LLM_AL] & [CASE_LLM_ERH] \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Algebraic Precision (Singularities)}: Primes are defined as singularities on the moral manifold where ethical curvature (Laplacian of the error landscape) exceeds a threshold. Manifold roughness was measured at [MANIFOLD_ROUGHNESS].
    ...
    \item \textbf{Dynamic ERH (Ethical Drift)}: A real-time monitor tracks the drift of $\alpha$ over time. In a simulated decay scenario, the monitor successfully issued a "CRITICAL" alert when $\alpha$ drifted to [DRIFT_ALPHA_FINAL].
...
% ============================================
% SECTION 9: AI ETHICS APPLICATIONS
% ============================================
