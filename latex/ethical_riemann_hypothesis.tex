...
\begin{itemize}
    \item \textbf{Empirical expansion}: prioritize a small set of high-stakes domains with reproducible data and interpretable complexity proxies, such as content moderation, medical triage, and benefit-eligibility screening.
    \item \textbf{Causal diagnostics}: connect large or unstable $\alpha$ values to specific data, modeling, or pipeline choices rather than treating ERH as a black-box score.
    \item \textbf{Training-time constraints}: explore ERH-inspired regularization as a design proposal, not yet a validated component of this repository.
    \item \textbf{Theory extensions}: formalize multi-dimensional and dynamic variants of ERH while keeping the current ethical-zeta construction explicitly heuristic.
\end{itemize}
