...
\begin{itemize}
    ...
    \end{itemize}
	    \item \textbf{圖表生成}：論文圖表由下列腳本自動生成。
	    \begin{quote}\ttfamily\small
	    simulation/generate\_all\_figures.py\\
	    scripts/run\_quantum\_hilbert\_figures.py
	    \end{quote}
	    PDF 圖表保存在 \path|simulation/output/figures/|。
	    \par\noindent 量子電路 PNG：\path|latest_quantum_circuit.png|。
	    \par\noindent 分布 PNG：\path|latest_quantum_distribution.png|。
    \item \textbf{環境依賴}：所有必要的 Python 套件列於 \texttt{requirements.txt}，建議使用 Python 3.10 或更高版本。
    \par\noindent 主要依賴包括 \texttt{numpy}、\texttt{scipy}、\texttt{matplotlib}、\texttt{pandas} 與 \texttt{jupyter}（用於 notebook 版本）。
...
\textbf{取得與轉換步驟}：
\begin{enumerate}
    \item 執行 \path|scripts/fetch_real_data.sh| 下載 Adult 與 COMPAS 至 \path|data/|。
    \par\noindent Adult 來源：\href{https://archive.ics.uci.edu/ml/machine-learning-databases/adult/}{UCI Adult dataset 頁面}。
    \par\noindent COMPAS 來源：\href{https://github.com/propublica/compas-analysis}{ProPublica COMPAS repository}。
...
\begin{itemize}
	    \item Adult Income：
	    \par\noindent \path|data/adult.csv|
	    \par\noindent \path|simulation/real_data/adult_income_case_study.py|
	    \par\noindent 需含 \texttt{income} 欄位
	    \item COMPAS：
	    \par\noindent \path|data/compas-scores-two-years.csv|
	    \par\noindent \path|simulation/real_data/compas_case_study.py|
	    \par\noindent 需含 \texttt{two\_year\_recid} 與 \texttt{decile\_score} 欄位
\end{itemize}
