...
\defaultCJKfontfeatures{AutoFakeSlant=0.2}
...
\begin{itemize}
    ...
    \end{itemize}
	    \item \textbf{圖表生成}：論文圖表由 \path|simulation/generate_all_figures.py| 與 \path|scripts/run_quantum_hilbert_figures.py| 自動生成。PDF 圖表保存在 \path|simulation/output/figures/|；量子電路與分布 PNG（\path|latest_quantum_circuit.png|、\path|latest_quantum_distribution.png|）由量子 Hilbert 模擬產生。
...
\textbf{輸出路徑與案例對應}：
\begin{itemize}
	    \item \path|data/adult.csv| $\rightarrow$ \path|simulation/real_data/adult_income_case_study.py|（需含 \texttt{income} 欄位）
	    \item \path|data/compas-scores-two-years.csv| $\rightarrow$ \path|simulation/real_data/compas_case_study.py|（需含 \texttt{two\_year\_recid}、\texttt{decile\_score} 欄位）
\end{itemize}
