...
\begin{figure}[htbp]
  ...
  \IfFileExists{../figures/universal_error_growth.png}{
    ...
    \IfFileExists{../simulation/output/figures/universal_error_growth.png}{
      ...
      \fbox{\parbox{0.75\textwidth}{\centering 圖 6：COMPAS 與模擬判斷者的誤差增長比較。執行 \texttt{scripts/generate\_comprehensive\_report.py} 以生成。模擬智能體曲線與 COMPAS 真實數據同繪於對數-對數圖，用於比較其結構性誤差累積型態。}}
    }
  }
...
除了 Adult Income 之外，我們還保留了兩個方法論層面的延伸案例：其一是「大學入學考試作弊」情境，其二是「性侵／性騷擾通報」情境。\footnote{對應程式分別位於 \texttt{simulation/real\_data/exam\_cheating\_case\_study.py} 與 \texttt{simulation/real\_data/sexual\_abuse\_case\_study.py}，僅適用於匿名化或合成資料。}
這些案例目前應視為分析流程的可移植性示範，而非額外的實證驗證。它們的合理用途，是協助研究者在高風險場景中檢查「錯誤是否會隨複雜度持續累積」；是否可用於部署決策，仍需依賴領域資料品質、傷害模型與既有公平性審查。
...
\begin{table}[htbp]
\centering
\caption{20 個模擬案例的彙整結果。各項指標為該類別下所有案例的平均值。[待填入] 將由模擬管線自動填入。}
\label{tab:20_cases_summary_zh}
\begin{tabular}{lcccc}
\toprule
\textbf{類別} & \textbf{錯誤率} & \textbf{質數數量} & \textbf{$\alpha$} & \textbf{滿足 ERH} \\
\midrule
司法與安全     & [CASE_JUD_MR] & [CASE_JUD_PR] & [CASE_JUD_AL] & [CASE_JUD_ERH_ZH] \\
醫療倫理       & [CASE_MED_MR] & [CASE_MED_PR] & [CASE_MED_AL] & [CASE_MED_ERH_ZH] \\
金融與分配     & [CASE_FIN_MR] & [CASE_FIN_PR] & [CASE_FIN_AL] & [CASE_FIN_ERH_ZH] \\
人力資源       & [CASE_HR_MR]  & [CASE_HR_PR]  & [CASE_HR_AL]  & [CASE_HR_ERH_ZH] \\
數位治理       & [CASE_GOV_MR] & [CASE_GOV_PR] & [CASE_GOV_AL] & [CASE_GOV_ERH_ZH] \\
教育學術       & [CASE_EDU_MR] & [CASE_EDU_PR] & [CASE_EDU_AL] & [CASE_EDU_ERH_ZH] \\
新興風險 (LLM) & [CASE_LLM_MR] & [CASE_LLM_PR] & [CASE_LLM_AL] & [CASE_LLM_ERH_ZH] \\
\bottomrule
\end{tabular}
\end{table}

模擬結果證實，雖然絕對錯誤率隨領域複雜度有所波動（例如在語境極其複雜的「仇恨言論審核」中較高），但誤差的結構性增長在所有 20 個場景中皆嚴格受限，$\alpha$ 指數始終低於 $0.5$ 的臨界閾值。
...
\begin{itemize}
    \item \textbf{代數精確化（奇點定義）}：將倫理質數定義為道德流形上的「奇點」，即誤差地景的拉普拉斯算子（曲率）超過閾值的點。實測流形粗糙度（Roughness）為 [MANIFOLD_ROUGHNESS]。
    \item \textbf{對抗性倫理質數}：利用模擬退火算法，識別能誘發結構性不穩定的針對性複雜度擾動，為系統提供「紅隊測試」基準。
    \item \textbf{動態 ERH（倫理漂移）}：開發了實時監控器追蹤 $\alpha$ 隨時間的偏移。在模擬的衰減場景中，當 $\alpha$ 漂移至 [DRIFT_ALPHA_FINAL] 時，監控器成功發出「CRITICAL（關鍵）」警告。
