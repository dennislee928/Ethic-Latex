...
**Architecture:** `erh_core` is the Single Source of Truth for core logic. The backend (`erh-security-app`) installs the root package in editable mode (`-e ../..`) to use the latest core. `simulation` and `erh` re-export from `erh_core` when available.
