...
\section{Quantum Simulation of Ethical Stability}
\label{sec:quantum}

To evaluate the stability of social consensus under the Ethical Riemann Hypothesis, we utilize a Variational Quantum Eigensolver (VQE) framework. The social interactions are mapped to a Quantum Ising Hamiltonian, and the system is evolved through a parameterized quantum circuit to find energy minima (consensus states).

\begin{figure}[h]
    \centering
    \IfFileExists{simulation/output/figures/quantum_circuit_step_latest.png}{%
        \includegraphics[width=0.8\textwidth]{simulation/output/figures/quantum_circuit_step_latest.png}%
    }{%
        \fbox{\parbox{0.7\textwidth}{\centering Run simulation with \texttt{enable\_quantum=True} to generate this figure.}}
    }
    \caption{The parameterized quantum circuit (Ansatz) representing the entangled ethical states of $N$ agents. The $R_y$ gates represent fluctuating individual stances, while $C_Z$ gates model the entanglement (social influence) between agents.}
    \label{fig:quantum_circuit}
\end{figure}

The ``Social Tension Energy'' $E_{\mathrm{soc}}$ is calculated as the expectation value of the Hamiltonian:
\begin{equation}
    E_{\mathrm{soc}} = \langle \psi(\theta) \mid H_{\mathrm{Ising}} \mid \psi(\theta) \rangle
\end{equation}
Minimizing this energy corresponds to locating the ``zeros'' in the ethical manifold.
