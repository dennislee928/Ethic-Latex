...
The ERH states that $|E(x)| \leq C \cdot x^{1/2 + \varepsilon}$ for some constants $C, \varepsilon > 0$, suggesting that errors in ``healthy'' judgment systems grow at most like $\sqrt{x}$. We present computational simulations comparing various judgment strategies (biased, noisy, conservative, radical) and demonstrate that different systems exhibit markedly different error growth patterns. Empirical validation on real-world datasets (COMPAS recidivism, Adult Income) yields $\alpha \approx -0.20$ for COMPAS and comparable exponents for Adult, supporting the applicability of ERH to algorithmic fairness evaluation. Applying this framework to the COMPAS dataset, we find that real-world judicial algorithms exhibit structural similarity to our Conservative judge ($\alpha \approx -0.14$)---errors accumulate sublinearly and satisfy the ERH bound.
...
Our empirical analysis yields $\alpha \approx -0.20$ for COMPAS. With $\alpha \approx -0.20$, COMPAS falls between our Radical ($\alpha \approx -0.31$) and Conservative ($\alpha \approx -0.14$) models, structurally similar to a moderately conservative judge. This indicates that COMPAS error growth is \emph{better than} the ERH bound ($\alpha < 0.5$): errors accumulate more slowly than $\sqrt{x}$ as complexity increases. However, this does not imply fairness---ERH characterizes structural stability of error growth, not demographic parity or calibration. The COMPAS case study demonstrates that ERH-style analysis can be applied to real-world algorithmic systems to quantify how judgment errors scale with case complexity.
...
\begin{figure}[h!]
    ...
    \caption{COMPAS: Adherence to Riemann Bound. Log-log plot of $|E(x)|$ vs.\ complexity $x$ with theoretical bound $C \cdot x^{0.5}$.}
...
\begin{figure}[h!]
    ...
    \caption{Structural similarity: $\alpha$ comparison across Radical, Conservative (synthetic), COMPAS, and Adult Income (real-world). COMPAS $\approx -0.20$ highlighted.}
...
Values are computed by running \texttt{scripts/run\_empirical\_validation.py}, \texttt{simulation/real\_data/process\_github.py}, and \texttt{simulation/real\_data/process\_huggingface\_llm.py}. The HuggingFace LLM module uses a free local model (DistilGPT-2, \textasciitilde80MB) via HuggingFace Transformers---no API key required. Even state-of-the-art LLMs exhibit error patterns consistent with the ERH bound when tested on the Moral Stories dataset. The empirical validation plot (Figure~\ref{fig:empirical-comparison}) overlays the theoretical bound $C \cdot x^{1/2}$, the Radical simulation curve, COMPAS data, and GitHub PR data, demonstrating that real-world curves fall within the theoretical bounds.
...
\begin{figure}[h!]
    ...
    \caption{Empirical validation: Theoretical bound ($C \cdot x^{1/2}$), Radical simulation, COMPAS, and GitHub PR. The overlay of COMPAS with simulated curves provides visual evidence that our framework captures real-world error growth patterns; all fall within ERH bounds.}
...
Our empirical validation on the COMPAS dataset demonstrates that real-world judicial algorithms exhibit $\alpha \approx -0.20$, satisfying the ERH bound and showing structural similarity to our Conservative judge model ($\alpha \approx -0.14$). The overlay of simulated and COMPAS error curves (Figure~\ref{fig:empirical-comparison}) provides visual evidence that our theoretical framework captures real-world error growth patterns.
