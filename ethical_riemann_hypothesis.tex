...
\begin{itemize}
    \item \textbf{Principle conflict count}: $c(a)$ = number of conflicting ethical principles (e.g., Deontology vs.\ Utilitarianism) involved in the action. High $c$ indicates moral dilemmas with incommensurable values.
    \item \textbf{Token-length proxy}: When action descriptions are available (e.g., from RLHF prompts), we use $\log(1 + \text{word count})$ as a proxy for reasoning complexity.
...
As conflict density increases, the system cannot reach a stable ground state: ground-state fidelity drops, and coherence collapses. This ``moral phase transition'' corresponds to the \emph{poles} of the Ethical Zeta Function---points where error explodes. We run this on IBM Quantum (or Aer simulator) and measure fidelity and Von Neumann entropy across conflict sweeps.
...
\begin{figure}[h!]
    ...
    \IfFileExists{simulation/output/phase_transition_diagram.png}{%
        ...
        \fbox{\parbox{0.7\textwidth}{\centering Run \texttt{scripts/run\_quantum\_phase\_transition.py --save-plot} to generate this figure.}}
    }
