...
% Use a widely available CJK font on cloud runners; if missing locally,
% you may switch this to a system font such as "Microsoft JhengHei" or "PingFang TC".
...
我們將「倫理質數」（ethical primes）定義為無法簡化為更簡單案例的基本誤判，並引入$\Pi(x)$作為複雜度級別$x$以下的倫理質數計數函數。透過建立類似於對數積分的基準期望$B(x)$，我們藉由誤差項$E(x) = \Pi(x) - B(x)$來表徵系統性偏差。ERH 指出對於常數$C, \varepsilon > 0$，有$|E(x)| \leq C \cdot x^{1/2 + \varepsilon}$，這表明「健康」判斷系統中的錯誤最多以次線性方式增長。
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{圖表生成}：論文圖表由 \texttt{simulation/generate\_all\_figures.py} 與 \texttt{scripts/run\_quantum\_hilbert\_figures.py} 自動生成。PDF 圖表保存在 \texttt{simulation/output/figures/}；量子電路與分布 PNG（\texttt{latest\_quantum\_circuit.png}、\texttt{latest\_quantum\_distribution.png}）由量子 Hilbert 模擬產生。
...
\midrule
Adult Income（訓練） & UCI ML Repository & \texttt{data/real\_world/adult.data} \\
Adult Income（測試） & UCI ML Repository & \texttt{data/real\_world/adult.test} \\
Adult Income（處理後） & --- & \texttt{data/adult.csv}（轉換後） \\
...
\textbf{取得與轉換步驟}：
\begin{enumerate}
    \item 執行 \texttt{scripts/fetch\_real\_data.sh} 下載 Adult（\url{https://archive.ics.uci.edu/ml/machine-learning-databases/adult/}）與 COMPAS（\url{https://github.com/propublica/compas-analysis}）至 \texttt{data/}。
...
\textbf{輸出路徑與案例對應}：
\begin{itemize}
    \item \texttt{data/adult.csv} $\rightarrow$ \texttt{simulation/real\_data/adult\_income\_case\_study.py}（需含 \texttt{income} 欄位）
    \item \texttt{data/compas-scores-two-years.csv} $\rightarrow$ \texttt{simulation/real\_data/compas\_case\_study.py}（需含 \texttt{two\_year\_recid}、\texttt{decile\_score} 欄位）
\end{itemize}
...
\begin{figure}[h!]
    ...
    \IfFileExists{simulation/output/figures/latest_quantum_circuit.png}{%
        ...
        \fbox{\parbox{0.7\textwidth}{\centering 執行 \texttt{scripts/run\_quantum\_hilbert\_figures.py} 或 \texttt{enable\_quantum=True} 模擬以生成此圖。}}
    }}
...
\begin{figure}[h!]
    ...
    \IfFileExists{simulation/output/figures/latest_quantum_distribution.png}{%
        ...
        \fbox{\parbox{0.7\textwidth}{\centering 執行 \texttt{scripts/run\_quantum\_hilbert\_figures.py} 以生成此圖。}}
    }}
...
\begin{table}[htbp]
\centering
\caption{量子 Hilbert 空間模擬指標。共識態為最常觀測的基底態；系統一致性衡量同意強度；Von Neumann 熵量化社會糾纏度。}
...
在進入各個判斷者的詳細敘述之前，表~\ref{tab:summary_judges_zh} 統整了每種類型在主要指標上的表現。

\begin{table}[htbp]
\centering
\caption{各判斷者類型的誤差行為總覽。錯誤率與平均 $|\Delta(a)|$ 皆在 $N = 2000$ 個行動上計算。「是否在 ERH 式界限內？」表示估計增長指數 $\alpha$ 是否不超過 ERH 風格的最壞情況目標（約 $\alpha \approx 0.5$）。「整體評估」提供結合結構健康與整體錯誤水準的簡潔評估。}
\label{tab:summary_judges_zh}
\begin{tabular}{lcccccp{5cm}}
\toprule
\textbf{判斷者類型} & \textbf{錯誤率} & \textbf{平均 $|\Delta(a)|$} & \textbf{$\alpha$} & \textbf{是否在 ERH 式界限內？} & \textbf{整體評估} & \textbf{質性評估} \\
\midrule
偏見判斷者       & 0.132 & 0.185 & $-0.629$ & 是 & 結構安全但具偏見 & 結構上非常安全（$|E(x)|$ 隨複雜度快速下降），但在某些區域仍呈現系統性偏見。 \\
隨機判斷者       & 0.256 & 0.209 & $-0.171$ & 是 & 結構安全但錯誤率高 & 誤差增長緩慢，但整體錯誤率與倫理質數數量偏高，顯示中等複雜度區域受雜訊主導。 \\
保守判斷者       & 0.611 & 0.342 & $-0.046$ & 是 & 結構安全但錯誤率過高不可用 & 誤差增長幾乎為零（沒有結構性爆炸），但整體錯誤水準極高，在道德上難以接受。 \\
激進判斷者       & 0.149 & 0.183 & $-0.452$ & 是 & 結構安全，表現可接受 & 誤差增長相當溫和且錯誤率較低，但在簡單案例上的極端行為仍可能帶來倫理風險。 \\
量子判斷者       & 0.750 & 0.995 & $-0.037$ & 是 & 結構安全但錯誤率過高 & 量子疊加態判斷；數值由模擬腳本填入。 \\
\bottomrule
\end{tabular}
\end{table}
...
為了說明 ERH 不僅適用於合成模擬，我們在 UCI Adult Income 資料集上進行了一個小型實證案例研究。\footnote{為便於可重現性，資料取得步驟請見第~\ref{subsec:real_world_data}小節。案例研究程式位於 \texttt{simulation/real\_data/adult\_income\_case\_study.py}。}
...
\begin{figure}[htbp]
  ...
  \IfFileExists{figures/universal_error_growth.png}{
    ...
    \IfFileExists{simulation/output/figures/universal_error_growth.png}{
      ...
      \fbox{\parbox{0.75\textwidth}{\centering 圖 6：普適誤差增長定律。執行 \texttt{scripts/generate\_comprehensive\_report.py} 以生成。模擬智能體曲線與 COMPAS 真實數據同繪於對數-對數圖。當兩條線重合時，理論的普適性得以視覺化驗證。}}
    }
  }
  \caption{普適誤差增長定律。模擬智能體曲線（偏見判斷者）與 COMPAS 真實數據同繪於對數-對數圖。當兩條線重合時，理論的普適性得以視覺化驗證。}
  \label{fig:universal_error_growth}
\end{figure}
% COMPAS 流程圖表將由 integrate_figures.py 插入此處

除了 Adult Income 之外，我們還設計了兩個方法論層面的延伸案例：其一是「大學入學考試作弊」情境，用於分析高風險測驗環境中的誠信偵測結構性風險；其二是「性侵／性騷擾通報」情境，用於探討在高度敏感領域中，通報與未通報之間的結構性脆弱性。\footnote{對應程式分別位於 \texttt{simulation/real\_data/exam\_cheating\_case\_study.py} 與 \texttt{simulation/real\_data/sexual\_abuse\_case\_study.py}，僅適用於匿名化或合成資料。}
在這兩個案例中，ERH 式分析同樣不取代傳統精度與公平性指標，而是補充了一個「錯誤如何隨案例複雜化而結構性累積」的宏觀視角，特別適合用於評估全球考試誠信與敏感社會議題中的高風險決策支援系統。
...
倫理質數的概念強調並非所有道德錯誤都是相等的。某些誤判是「基本的」，因為它們無法簡化為更簡單的組件錯誤。這與義務論倫理學產生共鳴，其中某些道德原則被認為是不可約的。

\textbf{應用情境反思}：倫理質數的概念在 AI 系統審計中最為有用，例如在累犯預測或醫療診斷系統中，某些關鍵誤判（如對特定人口群體的高重要性誤判）可能導致結構性不平等，即使整體誤差率較低。然而，在高度情境依賴的道德判斷中（例如自動駕駛的道德困境），「不可約性」的定義本身可能受到質疑，因為這些判斷往往涉及多個相互競爭的道德原則。在此情境下，ERH 框架主要提供一種\emph{隱喻框架}，幫助我們思考哪些誤判最值得優先處理，而非提供精確的數學分類。

\subsection{倫理質數作為關鍵失敗的指標}

倫理質數的識別為優先處理哪些錯誤提供了定量方法。這對 AI 系統審計和改進具有實際意涵，引導資源朝向最結構性重要的失敗。

\textbf{應用情境反思}：在實際的 AI 系統開發與維護中，ERH 框架可作為資源分配的定量工具。例如，在內容審核系統中，識別出導致大量後續誤判的「倫理質數」錯誤（如對特定語言或文化背景內容的系統性誤判），可以優先分配人力與計算資源進行修正。然而，需要注意的是，實驗結果顯示即使是「不健康」的系統（如高錯誤率的保守判斷者）也可能在誤差增長模式上優於 ERH 預測，因此倫理質數識別應與整體誤差水平評估相結合，而非單獨依賴。在此意義上，ERH 框架提供的是\emph{診斷工具}與\emph{優先排序機制}，而非全面的品質評估標準。
...
\begin{itemize}
    ...
    \item \textbf{義務論}：倫理質數代表不可約的道德原則，對應康德式義務論中不可違反的道德律 \citep{kant1785}
    \item \textbf{美德倫理學}：ERH 測量判斷系統的「品格」，即系統在面對不同複雜度挑戰時的一致性與穩定性
\end{itemize}

\textbf{應用情境反思}：ERH 框架的哲學意義在於它提供了一個跨理論的定量工具，可以同時服務於結果主義、義務論與美德倫理學的目標。在實際應用中，這意味著 ERH 可以在不同倫理框架下使用：結果主義者可能關注 ERH 如何限制整體誤差累積，義務論者可能關注倫理質數的識別與修正，而美德倫理學的支持者可能關注系統「品格」的測量。然而，我們必須認識到 ERH 框架本身並不解決這些哲學立場之間的根本分歧，而是提供一種共同的定量語言來討論系統性倫理問題。在資源分配或推薦系統等實際應用中，ERH 主要作為一種\emph{分析框架}，幫助決策者從不同哲學角度評估系統品質，而非作為單一的倫理判斷標準。

\subsection{限制與效度威脅}

我們承認幾個限制與對效度的潛在威脅：
\begin{itemize}
    \item \textbf{高度理想化的模型}：我們使用合成行動、風格化的判斷者與明確給定的真實值 $V(a)$ 來建構框架，這些設定在真實世界中往往無法完全滿足。
    \item \textbf{與質數的類比是啟發性的而非精確對應}：倫理質數僅在結構層面借用了質數的語言，並不主張道德錯誤真的服從與數論相同的定理。
    \item \textbf{複雜度與倫理質數定義具主觀性}：框架假設複雜度可以量化，且倫理質數的選取依賴設計好的重要性分位數與結構性判準，這些選擇在哲學與實務上都具有爭議空間。
    \item \textbf{ERH 僅為必要條件}：ERH 類型的結構性誤差上界最多只能視為倫理系統的必要條件，而非道德可接受性的充分條件；如高錯誤率的保守判斷者即便在誤差增長上優於 ERH 界限，仍在實務上不可接受。
    \item \textbf{心理史學整合的層級限制}：心理史學的使用僅停留在概念與模擬層級，是一種幫助思考宏觀誤差結構與危機動力學的隱喻，而非對真實社會歷程的可驗證預測理論。
\end{itemize}
