...
\begin{itemize}
    ...
    \item \textbf{真實世界資料}：Adult Income（UCI）與 COMPAS（ProPublica）資料集可透過 \texttt{scripts/fetch\_real\_data.sh} 取得，該腳本會下載 \texttt{adult.data}、\texttt{adult.test} 至 \texttt{data/real\_world/}，並將 \texttt{compas-scores-two-years.csv} 下載至 \texttt{data/}。執行 \texttt{scripts/convert\_adult\_to\_csv.py} 可將 UCI Adult 格式轉換為 \texttt{data/adult.csv} 供 Adult Income 案例研究使用。COMPAS 資料集由 \texttt{simulation/real\_data/compas\_case\_study.py} 直接讀取。
...
為了說明 ERH 不僅適用於合成模擬，我們在 UCI Adult Income 資料集上進行了一個小型實證案例研究。\footnote{為便於可重現性，請執行 \texttt{scripts/fetch\_real\_data.sh} 下載資料，再執行 \texttt{scripts/convert\_adult\_to\_csv.py} 產生 \texttt{data/adult.csv}。案例研究程式位於 \texttt{simulation/real\_data/adult\_income\_case\_study.py}。}
