...
\begin{table}[h]
\centering
\caption{各判斷者類型的誤差行為總覽。錯誤率與平均 $|\Delta(a)|$ 皆在 $N = 2000$ 個行動上計算。「是否在 ERH 式界限內？」表示估計增長指數 $\alpha$ 是否不超過 ERH 風格的最壞情況目標（約 $\alpha \approx 0.5$）。}
\label{tab:summary_judges_zh}
\begin{tabular}{lccccp{6.5cm}}
\toprule
\textbf{判斷者類型} & \textbf{錯誤率} & \textbf{平均 $|\Delta(a)|$} & \textbf{$\alpha$} & \textbf{是否在 ERH 式界限內？} & \textbf{質性評估} \\
\midrule
偏見判斷者       & 0.132 & 0.185 & $-0.629$ & 是 & 結構上非常安全（$|E(x)|$ 隨複雜度快速下降），但在某些區域仍呈現系統性偏見。 \\
隨機判斷者       & 0.256 & 0.209 & $-0.171$ & 是 & 誤差增長緩慢，但整體錯誤率與倫理素數數量偏高，顯示中等複雜度區域受雜訊主導。 \\
保守判斷者       & 0.611 & 0.342 & $-0.046$ & 是 & 誤差增長幾乎為零（沒有結構性爆炸），但整體錯誤水準極高，在道德上難以接受。 \\
激進判斷者       & 0.149 & 0.183 & $-0.452$ & 是 & 誤差增長相當溫和且錯誤率較低，但在簡單案例上的極端行為仍可能帶來倫理風險。 \\
\bottomrule
\end{tabular}
\end{table}
